...
def absolute_value_scaling2(
    ...
    # 1) 显式有效掩码（至少过滤非有限与非正深度；Bonn 还可在外层把 gt==-1 置 NaN）
    valid = torch.isfinite(predicted_depth) & torch.isfinite(ground_truth_depth) & (ground_truth_depth > 0)
    if valid.sum() < 10:
        return float(s_init), float(t_init)
    predicted_depth = predicted_depth[valid]
    ground_truth_depth = ground_truth_depth[valid]
...
def depth_evaluation(
    ...
    if metric_scale:
        ...
        predicted_depth[predicted_depth < 1e-5] = 0
        s_init = (
            torch.median(ground_truth_depth) / torch.median(predicted_depth)
        ).item()
        # print(torch.median(ground_truth_depth), torch.mean(ground_truth_depth), '====1')
        # print(torch.median(predicted_depth), torch.mean(predicted_depth), '====2')
        # print(ground_truth_depth.min().item(), ground_truth_depth.max().item(), '====1====')
        # print(predicted_depth.min().item(), predicted_depth.max().item(), '====1====')
        ...
        predicted_depth = s * predicted_depth + t
        # print(s, t, '====2==================')
        # print(predicted_depth.min().item(), predicted_depth.max().item(), '====2====')
        # print(torch.median(predicted_depth), torch.mean(predicted_depth), '====3')

        # # pred, gt: torch.Tensor 同形状，同一张/同一段的对齐数据（先做你的一致 mask，或把无效点设为 NaN）
        # med_gt   = torch.nanmedian(ground_truth_depth)
        # med_pred = torch.nanmedian(predicted_depth).clamp_min(1e-6)
        # print(torch.median(ground_truth_depth), torch.median(predicted_depth), '====1')
        # print(torch.mean(ground_truth_depth), torch.mean(predicted_depth), '====2')
        # # 预缩放系数（把 pred 拉到和 gt 同一个量级）
        # s_pre = (med_gt / med_pred).item()
        # # 预缩放后的输入：现在两者的中位数都 ~ 同量级，L1 更容易收敛
        # pred_norm = predicted_depth * s_pre
        # # 让 LAD 只做小范围微调（把 s_init 设为 1.0、t_init 设为 0）
        # s_hat, t_hat = absolute_value_scaling2(
        #     pred_norm, ground_truth_depth,
        #     s_init=1.0, t_init=0.0,
        #     lr=1e-3,          # 动态范围已经对齐，lr 可以比 1e-4 稍大一些
        #     max_iters=300
        # )
        # # 还原到原始尺度：最终 s、t
        # s = s_hat * s_pre
        ...
        # # 对齐后的预测
        # predicted_depth = s * predicted_depth + t
        # print(torch.median(ground_truth_depth), torch.median(predicted_depth), '====3')
        # print(torch.mean(ground_truth_depth), torch.mean(predicted_depth), '====4')
