...
def main(args):
    if args.eval_dataset == "sintel":
        ...
        if full:
            depth_pathes = glob.glob(f"/workspace/data/kaichen/data/test/Sintel/training/depth/*/*.dpt")
            ...
            depth_pathes_folder = [
                f"/workspace/data/kaichen/data/test/Sintel/training/depth/{seq}" for seq in seq_list
            ]
            depth_pathes = []
        ...
        img_pathes_folder = [
            f"/workspace/data/kaichen/data/test/bonn/rgbd_bonn_dataset/rgbd_bonn_{seq}/rgb_110/*.png"
            for seq in seq_list
        ]
        img_pathes = []
        ...
        depth_pathes_folder = [
            f"/workspace/data/kaichen/data/test/bonn/rgbd_bonn_dataset/rgbd_bonn_{seq}/depth_110/*.png"
            for seq in seq_list
        ]
        depth_pathes = []
        for depth_pathes_folder_i in depth_pathes_folder:
    ...
    elif args.eval_dataset == "dyncheck":

        def depth_read(filename):
            depth_map = np.load(filename).astype(np.float32)
            return depth_map

        seq_list = ["apple", "backpack", "block", "handwavy", "haru-sit", "mochi-high-five", "paper-windmill", "pillow", "space-out", "spin", "sriracha-tree", "teddy", "wheel"]
        # seq_list = ["mochi-high-five"]#, "backpack", "block", "handwavy", "haru-sit", "mochi-high-five", "paper-windmill", "pillow", "space-out", "spin", "sriracha-tree", "teddy", "wheel"]
        img_pathes_folder = [
            f"/workspace/data/kaichen/data/test/DYNCHECK/{seq}/rgb/1x/0_*.png"
            for seq in seq_list
        ]
        img_pathes = []
        for img_pathes_folder_i in img_pathes_folder:
            img_pathes += glob.glob(img_pathes_folder_i)
        # img_pathes = sorted(img_pathes)
        img_pathes = sorted(img_pathes, key=lambda x: int(x.split("_")[-1].split(".")[0]))
    

        pred_pathes = []; depth_pathes = []
        for seq in seq_list:
            _pred_pathes_ = glob.glob(f"{args.output_dir}/{seq}/frame*.npy")
            pred_pathes += sorted(_pred_pathes_, key=lambda x: int(x.split("_")[-1].split(".")[0]))

            _depth_pathes_ = glob.glob(f"/workspace/data/kaichen/data/test/DYNCHECK/{seq}/depth/1x/0_*.npy")
            depth_pathes += sorted(_depth_pathes_, key=lambda x: int(x.split("_")[-1].split(".")[0]))[:len(_pred_pathes_)]

        # pred_pathes = sorted(pred_pathes)
        # print(len(pred_pathes), '==================3')
        # print(len(depth_pathes), '==================4')
        def get_video_results():
            grouped_pred_depth = group_by_directory(pred_pathes)
            grouped_gt_depth = group_by_directory(depth_pathes, idx=-3)
            gathered_depth_metrics = []
            # print(len(grouped_gt_depth), '==================0', len(grouped_pred_depth))
            # print(grouped_gt_depth.keys(), '==================1')
            # print(grouped_pred_depth.keys(), '==================2')
            for key in tqdm(grouped_gt_depth.keys()):
                # print(key, '==================')
                pd_pathes = grouped_pred_depth[key]
                gt_pathes = grouped_gt_depth[key]
