...
import json
def dyncheck_cam_read(filename):
    with open(filename, 'r') as f:
        cam_data = json.load(f)
    position = np.array(cam_data.get('position', [0, 0, 0]))
    orientation = np.array(cam_data.get('orientation', np.eye(3)))
    extrinsic = np.block(
        [orientation, -orientation @ position[:, None]],).astype(np.float32)
    return extrinsic

def load_dyncheck_traj(gt_file): 
    # Refer to ParticleSfM
    gt_pose_lists = sorted(os.listdir(gt_file), key=lambda x: int(x.split("_")[-1].split(".")[0]))
    gt_pose_lists = [
        os.path.join(gt_file, x) for x in gt_pose_lists if x.endswith(".json") and "0_" in x]
    tstamps = [float(x.split("/")[-1].split("_")[-1].split(".")[0]) for x in gt_pose_lists]
    gt_poses = [
        dyncheck_cam_read(f) for f in gt_pose_lists]  # [1] means get the extrinsic
    xyzs, wxyzs = [], []
    tum_gt_poses = []
    for gt_pose in gt_poses:
        gt_pose = np.concatenate([gt_pose, np.array([[0, 0, 0, 1]])], 0)
...
def load_bonn_traj(gt_file):  
    # Refer to ParticleSfM
    print("================================================")
    print(gt_file); pdb.set_trace()
    gt_pose_lists = sorted(os.listdir(gt_file))
    gt_pose_lists = [
        os.path.join(gt_file, x) for x in gt_pose_lists if x.endswith(".cam")
    ]
...
def load_traj(gt_traj_file, traj_format="sintel", skip=0, stride=1, num_frames=None):
    ...
    elif traj_format == "dyncheck":
        traj_tum, timestamps_mat = load_dyncheck_traj(gt_traj_file)
    elif traj_format == "bonn":
        traj_tum, timestamps_mat = load_bonn_traj(gt_traj_file)
