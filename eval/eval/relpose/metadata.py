...
dataset_metadata = {
    ...
    "bonn": {
        "img_path": "/workspace/data/kaichen/data/test/bonn/rgbd_bonn_dataset",
        "mask_path": None,
    ...
    "dyncheck": {
        "img_path": "/workspace/data/kaichen/data/test/DYNCHECK",
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq, "rgb", "1x"),
        "gt_traj_func": lambda img_path, anno_path, seq: os.path.join(img_path, seq, "camera"),
        "traj_format": "dyncheck",
        "seq_list": [
            "apple", "backpack", "block", "handwavy", "haru-sit", 
            "mochi-high-five", "paper-windmill", "pillow", "space-out", 
            "spin", "sriracha-tree", "teddy", "wheel"
        ],
        # "seq_list": ["apple"],
        "full_seq": False,
        "mask_path_seq_func": lambda mask_path, seq: None,
        "skip_condition": None,
        "process_func": lambda args, img_path: process_dyncheck(args, img_path),
    },
    "nyu": {
    ...
    "tum": {
        "img_path": "/workspace/data/kaichen/data/test/tum",
        "mask_path": None,
    ...
    "sintel": {
        "img_path": "/workspace/data/kaichen/data/test/Sintel/training/final",
        "anno_path": "/workspace/data/kaichen/data/test/Sintel/training/camdata_left",
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq),
...
def process_dyncheck(args, img_path):
    if args.full_seq:
        for dir in tqdm(sorted(glob.glob(f"{img_path}/*/"))):
            filelist = sorted(glob.glob(f"{dir}/rgb/1x/0_*.png"))
            save_dir = f"{args.output_dir}/{os.path.basename(os.path.dirname(dir))}"
            yield filelist, save_dir
    else:
        seq_list = (
            ["apple", "backpack", "block", "handwavy", "haru-sit", "mochi-high-five", "paper-windmill", "pillow", "space-out", "spin", "sriracha-tree", "teddy", "wheel"]
            if args.seq_list is None
            else args.seq_list)
        for seq in tqdm(seq_list):
            filelist = sorted(glob.glob(f"{img_path}/{seq}/rgb/1x/0_*.png"))
            save_dir = f"{args.output_dir}/{seq}"
            yield filelist, save_dir
