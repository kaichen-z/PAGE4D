...
def main(args):
    if args.eval_dataset == "nyu":
        ...
        # seq_list = ["mochi-high-five"]
        depth_pathes_folder = [
            f"/workspace/data/kaichen/data/test/DYNCHECK/{seq}/depth/1x/0_*.npy"
            for seq in seq_list]
        depth_pathes = []
        for depth_pathes_folder_i in depth_pathes_folder:
            depth_pathes +=  sorted(glob.glob(depth_pathes_folder_i), key=lambda x: int(x.split("_")[-1].split(".")[0]))
        pred_pathes = []
        for seq in seq_list:
            pred_pathes_seq = glob.glob(
                f"{args.output_dir}/{seq}/*.npy")
            pred_pathes += sorted(pred_pathes_seq, key=lambda x: int(x.split("_")[-1].split(".")[0][:-5]))
    else:
        raise NotImplementedError
