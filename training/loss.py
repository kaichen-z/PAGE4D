...
from page.utils.pose_enc import extri_intri_to_pose_encoding
...
class MultitaskLoss(torch.nn.Module):
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        # Tracking loss - clean implementation following established patterns
...
def compute_camera_loss(
    ...
    **kwargs):
    ...
    valid_frame_mask = valid_frame_mask * (~batch_data['abandon_pose'])
...
def compute_point_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    if gt_points_mask.sum() < 100 or batch['abandon_geometry'].sum() > 0:
...
def compute_depth_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    if gt_depth_mask.sum() < 100 or batch['abandon_geometry'].sum() > 0:
        # If there are less than 100 valid points, skip this batch
...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1):
    ...
    safe_conf = torch.clamp(conf[mask], min=1e-6)
...
def normal_loss(prediction, target, mask, cos_eps=1e-8, conf=None, gamma=1.0, alpha=0.2):
    ...
    if loss.numel() < 10:
        ...
        if conf is not None:
            ...
            safe_conf = torch.clamp(conf, min=1e-6)
            loss = gamma * loss * safe_conf - alpha * torch.log(safe_conf)
            return loss.mean()
        else:
            return loss.mean()
...
def gradient_loss(prediction, target, mask, conf=None, gamma=1.0, alpha=0.2):
    ...
    if conf is not None:
        ...
        safe_conf_x = torch.clamp(conf_x, min=1e-6)
        safe_conf_y = torch.clamp(conf_y, min=1e-6)
        grad_x = gamma * grad_x * safe_conf_x - alpha * torch.log(safe_conf_x)
        grad_y = gamma * grad_y * safe_conf_y - alpha * torch.log(safe_conf_y)
...
def compute_track_loss(
    predictions,
    batch,
    gamma=0.8,           # temporal decay weight for multi-stage training
    weight_coord=1.0,    # weight for coordinate loss
    weight_vis=0.2,      # weight for visibility loss 
    weight_conf=0.1,     # weight for confidence loss
    vis_aware=False,     # whether to use visibility-aware loss
    vis_aware_w=0.1,     # weight for visibility awareness
    conf_threshold=3.0,  # pixel threshold for confidence ground truth
    **kwargs
):
    """
    Compute track loss following the same pattern as camera/depth/point losses.
    
    Args:
        predictions: Dict containing track predictions
        batch: Dict containing ground truth tracks and visibility masks
        gamma: Temporal decay weight for multi-stage training
        weight_coord: Weight for coordinate regression loss
        weight_vis: Weight for visibility prediction loss
        weight_conf: Weight for confidence prediction loss
        vis_aware: Whether to use visibility-aware coordinate loss
        vis_aware_w: Weight for visibility awareness in coordinate loss
        conf_threshold: Pixel threshold for generating confidence ground truth
    
    Returns:
        Dict containing individual track loss components
    """
    # Handle case where predictions don't contain track outputs
    if "track" not in predictions:
        dummy_loss = torch.tensor(0.0, device=batch["images"].device, requires_grad=True)
        return {
            "loss_coord_track": dummy_loss,
            "loss_vis_track": dummy_loss, 
            "loss_conf_track": dummy_loss,
        }
    # Extract predictions - prefer track_list for multi-stage training, fall back to track
    if "track_list" in predictions:
        coord_preds = predictions["track_list"]  # List of predictions from all iterations
    else:
        coord_preds = predictions["track"]       # Single prediction for backward compatibility
        # Convert single prediction to list for consistent handling
        if not isinstance(coord_preds, list):
            coord_preds = [coord_preds]
    
    # Extract visibility and confidence predictions
    vis_scores = predictions.get("vis", None)
    conf_scores = predictions.get("conf", None)
    
    # Extract ground truth - following official format expectation
    gt_tracks = batch["tracks"]              # Expected format: B, S, N, 2
    gt_track_vis_mask = batch["track_vis_mask"]  # Expected format: B, S, N
    
    # Get the number of predicted query points (following official logic)
    train_query_points = coord_preds[-1].shape[2]

    # Slice GT to match the number of prediction points (official approach)
    ...
    # Create validity mask - only consider tracks visible in first frame (official logic)
    valids = torch.ones_like(gt_track_vis_mask)
    mask = gt_track_vis_mask[:, 0, :] == True
    valids = valids * mask.unsqueeze(1)
    
    # Early return if no valid tracks (with debug output like official code)
    if not valids.any():
        ...
        # Create dummy losses that maintain gradients (following official pattern)
        dummy_coord = coord_preds[0].mean() * 0  # keeps graph & grads
        if vis_scores is not None:
            dummy_vis = vis_scores.mean() * 0
        else:
            dummy_vis = dummy_coord
        if conf_scores is not None:
            dummy_conf = conf_scores.mean() * 0
        else:
            dummy_conf = dummy_coord
        
        # Compute dummy total track loss
        dummy_total = dummy_coord * weight_coord + dummy_vis * weight_vis + dummy_conf * weight_conf
            
        return {
            "loss_track": dummy_total,
            "loss_coord_track": dummy_coord,
            "loss_vis_track": dummy_vis,
            "loss_conf_track": dummy_conf,
        }

    # Compute coordinate regression loss using sequence_loss (official logic)
    ...
    coord_loss = check_and_fix_inf_nan(coord_loss, "coord_loss", hard_max=None)
    
    # Compute visibility loss (following official approach)
    if vis_scores is not None:
        ...
        vis_loss = coord_loss * 0  # zero loss maintaining gradients
    
    # Compute confidence loss (following official approach - within 3 pixels)
    if conf_scores is not None:
        gt_conf_mask = (gt_tracks - coord_preds[-1]).norm(dim=-1) < conf_threshold
        conf_loss = F.binary_cross_entropy_with_logits(
            conf_scores[valids],
            gt_conf_mask[valids].float())
        conf_loss = check_and_fix_inf_nan(conf_loss, "conf_loss", hard_max=None)
    else:
        conf_loss = coord_loss * 0  # zero loss maintaining gradients
    
    # Compute total weighted track loss (following camera loss pattern)
    total_track_loss = (
        coord_loss * weight_coord +
        vis_loss * weight_vis +
        conf_loss * weight_conf
    )
    
    # Return loss dictionary following the pattern of other loss functions
    return {
        "loss_track": total_track_loss,
        "loss_coord_track": coord_loss,
        "loss_vis_track": vis_loss,
        "loss_conf_track": conf_loss,
    }
...
def sequence_loss(flow_preds, flow_gt, vis, valids, gamma=0.8, vis_aware=False, huber=False, delta=10, vis_aware_w=0.1, **kwargs):
    ...
    Computes L1 loss between predicted and ground truth tracks across multiple prediction stages.
    
    Args:
        flow_preds: List of predicted tracks, each with shape (B, S, N, 2)
        flow_gt: Ground truth tracks (B, S, N, 2)
        vis: Visibility mask (B, S, N)
        valids: Validity mask (B, S, N) 
        gamma: Temporal decay weight for multi-stage predictions
        vis_aware: Whether to use visibility-aware weighting
        huber: Whether to use Huber loss (unused but kept for compatibility)
        delta: Delta parameter for Huber loss (unused but kept for compatibility)
        vis_aware_w: Weight for visibility awareness
    
    Returns:
        Weighted average loss across all prediction stages
    """

    B, S, N, D = flow_gt.shape
    assert D == 2, "Track coordinates must be 2D (x, y)"
    B, S1, N = vis.shape
    B, S2, N = valids.shape
    assert S == S1
    assert S == S2
    ...
    for i in range(n_predictions):
        ...
        # Compute L1 loss between predicted and ground truth coordinates
        i_loss = (flow_pred - flow_gt).abs()  # B, S, N, 2
        i_loss = check_and_fix_inf_nan(i_loss, f"i_loss_iter_{i}", hard_max=None)
        i_loss = torch.mean(i_loss, dim=3)  # Average over x,y -> B, S, N
        
        # Combine validity and visibility masks
        combined_mask = torch.logical_and(valids, vis)
        num_valid_points = combined_mask.sum()
        
        if vis_aware:
            # Visibility-aware weighting: give higher weight to visible points
            ...
            # Standard approach: only compute loss on valid points
            if num_valid_points > 2:
                i_loss = i_loss[combined_mask]
                flow_loss += i_weight * i_loss.mean()
            else:
                # Not enough valid points, add zero loss that maintains gradients
...
def reduce_masked_mean(x, mask, dim=None, keepdim=False):
    """
    Compute masked mean, handling cases where mask sum is zero.
    
    Args:
        x: Tensor to compute mean over
        mask: Binary mask tensor
        dim: Dimensions to reduce over
        keepdim: Whether to keep reduced dimensions
    
    Returns:
        Masked mean, with zero where mask sum is zero
    """
