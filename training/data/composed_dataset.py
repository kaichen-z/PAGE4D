...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        if "temporal_features" in batch and len(batch["temporal_features"]) > 0:
            temporal_features = torch.from_numpy(batch["temporal_features"].astype(np.float32))
        ...
        if temporal_features is not None:
            sample["temporal_features"] = temporal_features
        # --- Track Processing (if enabled) ---
        if self.load_track:
            # Use pre-computed tracks from the dataset (now guaranteed to have consistent shape)
            ...
            # Tracks are already normalized to self.track_num by individual datasets
            # so we can directly use them without further sampling
            track_positive_mask = torch.ones(tracks.shape[1]).bool()
        ...
        sample["abandon_pose"] = batch["abandon_pose"]
        sample["abandon_geometry"] = batch["abandon_geometry"]
        return sample
...
class TupleConcatDataset(ConcatDataset):
    ...
    where the first element is used to determine which sample to fetch, and the rest are
    passed down to the selected dataset's __getitem__ method.
