...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.track_num = getattr(common_conf, 'track_num', 1024)

    def _discover_sequences_generic(
        self,
        ...
        """
        Generic sequence discovery method that can be configured for different dataset types.
        
        Args:
            dataset_location: Root directory to search
            required_subdirs: List of required subdirectories (e.g., ['rgbs', 'depths'])
            required_files: List of required files (e.g., ['anno.npz'])
            image_subdir: Subdirectory containing images (e.g., 'rgbs')
            image_pattern: Pattern for image files (e.g., 'rgb_*.jpg')
        ...
        if not os.path.exists(dataset_location):
            if verbose:
                print(f'Dataset location does not exist: {dataset_location}')
            return []
        ...
        for item in os.listdir(dataset_location):
            seq_path = os.path.join(dataset_location, item)
            ...
            for subdir in required_subdirs:
                if not os.path.isdir(os.path.join(seq_path, subdir)):
                    missing_subdirs.append(subdir)
            
            # Check for required files
            missing_files = []
            for req_file in required_files:
                if not os.path.isfile(os.path.join(seq_path, req_file)):
                    missing_files.append(req_file)
            
            if missing_subdirs or missing_files:
                if verbose:
                    missing_items = missing_subdirs + missing_files
                    print(f'  Skipping {item}: missing {missing_items}')
                continue
                
            # Check for sufficient image files
            image_path = os.path.join(seq_path, image_subdir)
            image_files = glob.glob(os.path.join(image_path, image_pattern))
            
            if len(image_files) >= min_num_images:
                discovered_sequences.append(item)
                if verbose:
                    print(f'  Found valid sequence: {item} ({len(image_files)} frames)')
            elif verbose:
                print(f'  Skipping {item}: insufficient frames ({len(image_files)})')
                
        discovered_sequences.sort()  # Sort for consistency
    ...
    def process_one_image(
        ...
        image, depth_map, intri_opencv, track, track_mask1 = crop_image_depth_and_intrinsic_by_pp(
        ...
        if self.rescale:
            image, depth_map, intri_opencv, track = resize_image_depth_and_intrinsic(
                ...
                resize_ours=resize_ours,
            )
        else:
            print("Not rescaling the images")

        # Ensure final crop to target shape
        image, depth_map, intri_opencv, track, track_mask2 = crop_image_depth_and_intrinsic_by_pp(
            image, depth_map, intri_opencv, target_shape, track=track, filepath=filepath, strict=True,
        )
        if track_mask1 is not None and track_mask2 is not None:
            track_mask = track_mask1 & track_mask2
        else:
            track_mask = None
