...
class RLBenchDataset(BaseDataset):
    """RLBench dataset loader for VGGT."""
    def __init__(
        self,
        common_conf,
        dataset_location: str = "/workspace/data/kaichen/robot/rlbench_flat",
        split: str = "val",
    ...
    def _discover_sequences(self) -> List[str]:
        ...
        for d in os.listdir(self.dataset_location):
            for sub_d in os.listdir(os.path.join(self.dataset_location, d)):
                if os.path.isdir(os.path.join(self.dataset_location, d, sub_d)):
                    seqs.append(d + '/' + sub_d)
        seqs.sort()
        if self.verbose:
    ...
    def _load_sequences(self): 
        ...
        for seq_name in self.sequence_names:
            ...
            rgb_dir   = os.path.join(seq_dir, "front_rgb_frames")
            depth_dir = os.path.join(seq_dir, "front_depth_npy")
            seq_dir = Path(seq_dir)
            seq_dir = seq_dir.parent

            gt_path   = os.path.join(seq_dir, "camera_params.json")
            ...
            with open(gt_path, "r") as f:
                pose_data = json.load(f).get("front", None)
            self.sequence_metadata[seq_name+"_front"] = {"num_frames": num_frames,
                "frame_indices": frame_indices, "rgb_files": rgb_files, 
                "depth_files": depth_files, "gt_rows": pose_data,}
            self._generate_clips_for_sequence(seq_name+"_front", num_frames, frame_indices)

            rgb_dir   = os.path.join(seq_dir, "overhead_rgb_frames")
            depth_dir = os.path.join(seq_dir, "overhead_depth_npy")
            gt_path   = os.path.join(seq_dir, "camera_params.json")
            # 1) read *.png and sort (name-sorted == time order in Bonn)
            rgb_files   = sorted(glob.glob(os.path.join(rgb_dir, "*.jpg")))
            depth_files = sorted(glob.glob(os.path.join(depth_dir, "*.npz")))
            ...
            num_frames = len(rgb_files)
            with open(gt_path, "r") as f:
                pose_data = json.load(f).get("overhead", None)
            self.sequence_metadata[seq_name+"_overhead"] = {"num_frames": num_frames,
                "frame_indices": frame_indices, "rgb_files": rgb_files, 
                "depth_files": depth_files, "gt_rows": pose_data,}
            self._generate_clips_for_sequence(seq_name+"_overhead", num_frames, frame_indices)
    ...
    def _load_camera_poses(self, seq_meta, frame_indices):
        extrinsic = np.array(seq_meta["Extrinsic"]).astype(np.float64)   # [ts tx ty tz qx qy qz qw]
        intrinsic = np.array(seq_meta["Intrinsic"]).astype(np.float64)   # [ts tx ty tz qx qy qz qw]
    ...
    def get_data(
        ...
        # Load camera poses
        extrinsic, intrinsic = self._load_camera_poses(seq_metadata["gt_rows"], frame_indices)
        ...
        # add ="/workspace/data/kaichen/data/RLbench/1776721200_front/front_depth_npy/0000.npz"
        # -------------------- Process Images -------------------------
        ...
        for i, fid in enumerate(frame_indices):
            ...
            (   proc_img,
                ...
                original_image,
                depth_map,
                extrinsic[:3, :],  # 3x4 extrinsic matrix
                intrinsic,
                original_size,
                target_shape,
