...
test_seq = ["sitting_halfsphere", "sitting_rpy", "sitting_static", "sitting_xyz", "walking_halfsphere", "walking_rpy", "walking_static", "walking_xyz"]
...
class TumDataset(BaseDataset):
    """TUM RGBD dataset loader for VGGT.
    ...
    def __init__(
        self,
        common_conf,
        dataset_location: str = "/workspace/data/kaichen/data/test/tum",
        split: str = "val",
        ...
        **kwargs,
        ...
        if self.split == "test" or self.split == "val":
            self.sequence_names = ["rgbd_dataset_freiburg3_" + i for i in test_seq]
        else:
        ...
        print(
            f"Loaded {len(self.sequence_names)} TUM sequences – "
    ...
    def _discover_sequences(self) -> List[str]:
        """Discover available TUM sequences."""
        ...
        seqs.sort()
        if self.verbose:
            print("[TUM] Discovered sequences:", seqs)
        return seqs
    ...
    def _load_sequences(self): 
        ...
        for seq_name in self.sequence_names:
            ...
            rgb_dir   = os.path.join(seq_dir, "rgb_90")
            depth_dir = os.path.join(seq_dir, "depth_90")
            gt_path   = os.path.join(seq_dir, "groundtruth_90.txt")
            # 1) read *.png and sort (name-sorted == time order in TUM)
            rgb_files   = sorted(glob.glob(os.path.join(rgb_dir, "*.png")))
            depth_files = sorted(glob.glob(os.path.join(depth_dir, "*.png")))
            # pose_data =  np.loadtxt(gt_path)
            pose_data =  np.loadtxt(gt_path)
            # print(len(rgb_files), len(depth_files), '===========rgb_files, depth_files===========111', len(pose_data))
            num_frames = 90
    ...
    def _load_camera_poses(self, seq_meta, frame_indices):
        ...
        for fid in frame_indices:
            ...
            T_w_m[:3,:3] = R
            T_w_m[:3, 3] = [tx, ty, tz]
            T_c_w     = np.linalg.inv(T_w_m).astype(np.float32)
    ...
    def get_data(
        ...
        # -------------------- Load depth maps -------------------------
        depths = []
        # print(frame_indices, '===========frame_indices===========', len(seq_metadata["depth_files"]))
        for i, fid in enumerate(frame_indices):
            # Find the corresponding file path for this frame index
        ...
        for i, fid in enumerate(frame_indices):
            ...
            # Estimate intrinsics
            K = self._estimate_intrinsics(original_image.shape)
            original_size = np.array(original_image.shape[:2])  # (H, W)
            # Process one image
            (   proc_img,
                proc_depth,
            ...
            # print(check, '===========check===========')
            # matrix_check(proc_img, proc_depth, proc_intri, proc_extri)
